...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$KeyFile = "security\survey-app-key.pem"
)
...
if (-not (Test-Path $KeyFile)) {
    $KeyFile = "$projectRoot\security\survey-app-key.pem"
    if (-not (Test-Path $KeyFile)) {
...
Write-Host "Destination: ubuntu@${Ec2Ip}:~/survey-web-app/" -ForegroundColor Gray
...
scp -i $KeyFile -r `
    ...
    "$projectRoot\backend" "ubuntu@${Ec2Ip}:~/survey-web-app/"
...
scp -i $KeyFile -r `
    -o "StrictHostKeyChecking=no" `
    "$projectRoot\frontend" "ubuntu@${Ec2Ip}:~/survey-web-app/"
...
Write-Host "  cd ~/survey-web-app" -ForegroundColor Gray
